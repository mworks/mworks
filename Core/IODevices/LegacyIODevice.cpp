...
bool LegacyIODevice::registerCapability(IOCapability *  capability){
...
int LegacyIODevice::matchCapabilityByName(std::string name){
...
LegacyIODevice::LegacyIODevice(const LegacyIODevice& copy){
...
IOChannel *  LegacyIODevice::makeNewChannel(
										IOChannelRequest *  _request, 
...
ExpandableList<IOPhysicalDeviceReference> *LegacyIODevice::getAvailablePhysicalDevices(){
...
bool LegacyIODevice::attachPhysicalDevice(){  
	return false; 
}         // attach next avaialble device

bool LegacyIODevice::attachPhysicalDevice(IOPhysicalDeviceReference* deviceToAttach){  // overriden versions will want to validate
...
IOPhysicalDeviceReference* LegacyIODevice::getAttachedPhysicalDevice() {            // allows the core to query to see which device this object is currently attached to (if any)
	return attached_device;
}

bool LegacyIODevice::isAttached(){
...
ExpandableList<IOCapability> *LegacyIODevice::getCapabilities(){
	if (!isAttached()) {       // if deviced not attached, return NULL pointer and error message
		mprintf("Error: LegacyIODevice::getCapabilities:  call to get capabilities without first attaching device.");
		return(NULL);
	} 
...
bool LegacyIODevice::requestChannel(shared_ptr<IOChannelRequest>  request){
...
void LegacyIODevice::addChild(std::map<std::string, std::string> parameters,
...
bool LegacyIODevice::initialize(){
...
bool LegacyIODevice::mapRequestsToChannels(){
    ...
    /*						
	 ...
	 mprintf("Error: LegacyIODevice::initializeChannels:  initializeChannels request made without first attaching device.");
	 return false;
	 } 
	 */
...
bool LegacyIODevice::validateIndividualChannel(
										  IOChannel *  tentative_channel, 
										  IOChannelIncompatibility* incompatibility) {
...
bool LegacyIODevice::initializeChannels(){
...
bool LegacyIODevice::startDeviceIO(){
	if (!isAttached())  {       // if deviced not attached, return false and error message
		mprintf("Error: LegacyIODevice::startDeviceIO:  startDeviceIO request made without first attaching device.");
		return false;
	} 
...
bool LegacyIODevice::startDeviceIOchannels() {
	...
	for(int i = 0; i < channels->getNElements(); i++){
		...
		if (the_channel->isInitialized()) {		// check that we have already initialized the cahnnel before we try to schedule its update method 
			...
			args->device = dynamic_pointer_cast<LegacyIODevice>(IODevice::shared_from_this());
...
bool LegacyIODevice::stopDeviceIO(){
	if (!isAttached())  {       // if deviced not attached, return false and error message
		mprintf("Error: LegacyIODevice::stopDeviceIO:  stopDeviceIO request made without first attaching device.");
		return false;
	} 
...
bool LegacyIODevice::updateChannel(int channel_index){ return false;}     

bool LegacyIODevice::updateChannel(int channel_index, Datum data){ 
	return updateChannel(channel_index);
}

bool LegacyIODevice::updateAllChannels(){
	for(int i = 0; i < channels->getNElements(); i++){
		updateChannel(i);
	}
	return true;
...
int LegacyIODevice::getChannelIndex(std::string channel_name){
	for(int i = 0; i < channels->getNElements(); i++){
		if(channel_name == (*channels)[i]->getName()){
			return i;
		}
	}
	mprintf("Error: LegacyIODevice::getChannelIndex:  could not find channel index from channel name.");
	return -1; // no such channel
}

// access channel by name  
IOChannel *  LegacyIODevice::getChannel(std::string channel_name){
	int channelIndex;
	channelIndex = getChannelIndex(channel_name);
	if (channelIndex >= 0) {
		return getChannel(channelIndex);
	} else {
		mprintf("Error: LegacyIODevice::getChannel(name):  could not find channel index from channel name.");
		return NULL;
	}    
}

// access channel by index  
IOChannel *  LegacyIODevice::getChannel(int channel_index){
	if ( (channel_index >= 0) || (channel_index < channels->getNElements()) ) {
		return ((*channels)[channel_index]).get();
	} else {
		mprintf("Error: LegacyIODevice::getChannel(int): channel index out of range.");
		return NULL;        
	}
}

ExpandableList<IOChannel> *LegacyIODevice::getChannels(){
	return channels;
}

ExpandableList<IOChannelIncompatibility> *LegacyIODevice::getIncompatibilities(){
	return incompatibilities;
}


bool LegacyIODevice::validateMultipleChannels(ExpandableList<IOChannel> *candidates){
	return candidates;
}



bool LegacyIODevice::deactivateChannel(std::string channel_name){ return false;}
bool LegacyIODevice::deactivateChannel(int channel_index){return false;}

bool LegacyIODevice::activateChannel(std::string channel_name){return false;}
bool LegacyIODevice::activateChannel(int channel_index){return false;}

bool LegacyIODevice::removeChannel(std::string channel_name){return false;}
bool LegacyIODevice::removeChannel(int channel_index){return false;}

bool LegacyIODevice::stopAllScheduleNodes(){
...
AsynchronousOutputNotification::AsynchronousOutputNotification( 
																 shared_ptr<LegacyIODevice> _device,
																 int _channel_index):
