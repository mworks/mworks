    ...
    EventTransport(_type, _dir)
{
    ...
    if (type == server_event_transport) {
        ...
        outgoing_queue = shared_ptr<message_queue>(new message_queue(boost::interprocess::create_only, resource_name_outgoing.c_str(), outgoing_queue_size, MAX_MESSAGE_SIZE));
        incoming_queue = shared_ptr<message_queue>(new message_queue(boost::interprocess::create_only, resource_name_incoming.c_str(), incoming_queue_size, MAX_MESSAGE_SIZE));
