...
bool dfrIsString(const char *tmp, int length);
int getScarabSystemEventType(ScarabDatum *payload);
int getScarabSystemEventControlType(ScarabDatum *payload);
std::string line();
std::string dfrScarabEventToEventStruct(const int index, 
					ScarabDatum *datum);
std::string dfrNiceNumber(const unsigned long num);
std::string dfrNumber(const long long num);
...
std::string getEventTypeName(const int type);
...
int main( int argc, char *argv[])
{
  ...
  if ( argc < 2 ) 
    cout << "usage: " << argv[0] << " filename [[events to include] [] [] [] ...]" << endl;
  else {
    ...
    if ( !the_file.is_open() )
      ...
      std::map<std::string, int> variables = dfrGetVariables(argv[1]);
	...
	for(std::map<std::string, int>::const_iterator i = variables.begin();
      ...
      ScarabSession *session = dfrConnectToFile(argv[1]);
      ScarabDatum *datum;
      int count = 0;
	...
	if(std::binary_search(eventsToInclude.begin(), eventsToInclude.end(), code)) {
	  cout << dfrScarabEventToEventStruct(count, datum);
	}
...
ScarabDatum *getScarabSystemEventPayload(ScarabDatum *payload){
  return scarab_list_get(payload, SCARAB_SYSTEM_EVENT_PAYLOAD_INDEX);
}

int getScarabSystemEventType(ScarabDatum *payload){

  ScarabDatum *type_datum = scarab_list_get(payload, 
					    SCARAB_SYSTEM_EVENT_PACKAGE_TYPE_INDEX);
...
int getScarabSystemEventControlType(ScarabDatum *payload){
  ...
  ScarabDatum *type_datum = scarab_list_get(payload, 
					    SCARAB_SYSTEM_EVENT_CONTROL_TYPE_INDEX);
  return type_datum->data.integer;
}
...
std::string scarabValue(ScarabDatum *payload) {
  ...
  if(payload->type == SCARAB_INTEGER){
    ...
    if(dfrIsString(tmp, payload->data.opaque.size)) {
      ret = ret + "STRING: " + tmp;
    } else {
      ret += "OPAQUE";
    }
...
std::string recursiveGetScarabDict(ScarabDatum *datum) {
  ...
  for(int i = 0; i < n; i++){
      ...
      case SCARAB_OPAQUE: 
	{
	  ...
	  if(dfrIsString(tmp, values[i]->data.opaque.size)) {
	    ret = ret + "(string)" + tmp;
	  } else {
	    ret += "(binary)";
	  }
...
std::string getEventTypeName(const int type) {
  ...
  switch(type) {
  case M_SYSTEM_CONTROL_PACKAGE:
    return std::string("M_SYSTEM_CONTROL_PACKAGE");
    break;
  case M_SYSTEM_DATA_PACKAGE:
    return string("M_SYSTEM_DATA_PACKAGE");
    break;
  case M_SYSTEM_RESPONSE_PACKAGE:
    return string("M_SYSTEM_RESPONSE_PACKAGE");
    break;
  default:
    break;
  }
