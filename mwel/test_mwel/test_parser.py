...
class TestStatements(ParserTestMixin, unittest.TestCase):
    ...
    def test_range_rep_stmt(self):
        def test_rr(src, colno, varname, start, stop, step=None, children=()):
            with self.parse(src) as p:
                self.assertIsInstance(p, ast.Module)
                self.assertEqual(1, len(p.statements))
                p = p.statements[0]

                self.assertIsInstance(p, ast.RangeRepStmt)
                self.assertLocation(p, 2, colno)
                self.assertEqual(varname, p.varname)
                self.assertEqual(start, p.start)
                self.assertEqual(stop, p.stop)
                self.assertEqual(step, p.step)
                self.assertIsInstance(p.children, tuple)
                self.assertEqual(children, p.children)
        ...
        test_rr('''
                ...
                children = (
                    ast.VarStmt(name='foo', value=self.bar),
                    ast.AssignmentStmt(varname='bar', value=self.two),
                    ast.DeclarationStmt(type = 'foo',
                                        tag = 'bar',
                                        params = (),
                                        children = ()),
        ...
        # Wrong case
        with self.parse('%FOR index = 1:3 \n %end'):
            self.assertError(token='%')

        # Missing variable name
        with self.parse('%for = 1:3 \n %end'):
            self.assertExpected('identifier', got='=')

        # Missing =
        with self.parse('%for index 1:3 \n %end'):
            self.assertExpected("'=' or 'in'", got='1')

        # Missing start
        with self.parse('%for index = :3 \n %end'):
            self.assertError(token=':')
        ...
        # Missing newline
        with self.parse('%for index = 1:3 %end'):
            self.assertExpected('line ending')
    ...
    def test_list_rep_stmt(self):
        def test_lr(src, colno, varname, items=(), children=()):
            with self.parse(src) as p:
                self.assertIsInstance(p, ast.Module)
                self.assertEqual(1, len(p.statements))
                p = p.statements[0]

                self.assertIsInstance(p, ast.ListRepStmt)
                self.assertLocation(p, 2, colno)
                self.assertEqual(varname, p.varname)
                self.assertIsInstance(p.items, tuple)
                self.assertEqual(items, p.items)
        ...
        test_lr('''
                ...
                items = (
                    self.foo,
                    ast.RangeExpr(start=self.one, stop=self.three),
                    self.bar,
                    ),
                children = (
                    ast.VarStmt(name='foo', value=self.bar),
        ...
        # Wrong case
        with self.parse('%FOR item in 1 \n %end'):
            self.assertError(token='%')

        # Missing variable name
        with self.parse('%for in 1 \n %end'):
            self.assertExpected("'=' or 'in'", got='1')

        # Missing 'in'
        with self.parse('%for item 1 \n %end'):
            self.assertExpected("'=' or 'in'", got='1')

        # Bad 'in'
        with self.parse('%for item IN 1 \n %end'):
            self.assertExpected("'=' or 'in'", got='IN')

        # No values
        with self.parse('%for item in \n %end'):
            self.assertError('Line ended unexpectedly', token='\n')

        # Extra comma
        with self.parse('%for item in 1,2, \n %end'):
            self.assertError('Line ended unexpectedly', token='\n')

        # Missing newline
        with self.parse('%for item in 1,2  %end'):
            self.assertExpected('line ending')
        ...
        # Bad %end
        with self.parse('%for item in 1:3 \n %endd'):
            self.assertError(token='%')
